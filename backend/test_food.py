...
from server.api.endpoints import food_bp
...
def test_search_food_route(client: FlaskClient, monkeypatch):
    ...
    monkeypatch.setattr(
        "server.api.external_api.fatsecret.search_food", mock_search_food
    )
    ...
    assert response_missing_name.get_json() == {
        "error": "Query param 'nome' is required"
    }
    ...
    monkeypatch.setattr(
        "server.api.external_api.fatsecret.search_food", mock_search_food_with_exception
    )
